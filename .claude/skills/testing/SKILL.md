...
For the RED-GREEN-REFACTOR workflow itself, load the `mattpocock-skills:tdd` skill. For Vitest seed semantics and reusable-command proof, read [resources/vitest-watch-feedback.md](resources/vitest-watch-feedback.md).
...
|---|---|---|
| Vitest | Repository-owned watcher first. Otherwise use `vitest --changed <real-base> --watch` once you have confirmed the repository's Vitest version and configuration support it; for one-shot use `--run`. Use `vitest related <all-changed-source-paths>` only when that list is derived mechanically | `vitest run <test-file> -t <name>` or exact-file watch |
